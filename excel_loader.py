...
SYNONYMS = {
    ...
    "programming":    ["computing", "software", "developer", "programming"],
    "java":           ["computing", "software", "programming", "developer"],
    "python":         ["computing", "software", "programming", "developer"],
}
...
def _expand_query(query: str) -> list:
    ...
    for word in re.findall(r"\b\w+\b", query_lower):
        if word not in ("the","a","an","is","are","do","does","can","any",
                        "for","in","of","and","or","to","me","my","i","this",
                        "that","it","be","was","with","on","at","by"):
            terms.append(word)
...
class CourseLoader:
    ...
    def format_course_for_bot(self, course: dict) -> str:
        """Rich summary card for search results."""
        def val(key):
        ...
        if val("Method of Assessment"):
            lines.append(f"Assessment: {_first_sentences(val('Method of Assessment'), 2)}")
        ...
        if val("Career Progression"):
            ...
            intro_lines  = [l for l in career_lines if "£" not in l and len(l) < 120][:1]
            if salary_lines:
    ...
    def format_full_course(self, course: dict) -> str:
        """
        Full details — Option 3 Clean Minimal.
        ALL-CAPS headings, → bullets, dot-aligned salary table.
        """
        def val(key):
            return course.get(key, "").strip()
        ...
        if val("Number of Credits"):    facts.append(f"{val('Number of Credits')}")
        ...
        if val("Course Overview"):
            ...
            lines.append(_first_sentences(val("Course Overview"), 3))
        ...
        if val("Career Progression"):
            ...
            if salary_lines:
                for sl in salary_lines[:6]:
                    sep = "–" if "–" in sl else ("-" if "-" in sl else None)
                    if sep:
                        parts = sl.split(sep, 1)
                        job = parts[0].strip()
                        sal = parts[1].strip() if len(parts) > 1 else ""
                        dots = "." * max(1, 30 - len(job))
                        lines.append(f"→ {job} {dots} {sal}")
                    else:
                        lines.append(f"→ {sl}")
            elif career_lines:
                lines.append(_bullet(val("Career Progression"), max_items=6))
            lines.append("")
        ...
        if val("Academic Progression"):
            ...
            qual_lines = [l for l in acad_lines if not l.lower().startswith("step")
                          and "£" not in l and 5 < len(l) < 100][:3]
            if step_lines:
                lines.append(_bullet("\n".join(step_lines[:5])))
            elif qual_lines:
                lines.append(_bullet("\n".join(qual_lines)))
            else:
                lines.append(_bullet(val("Academic Progression"), max_items=4))
            lines.append("")
